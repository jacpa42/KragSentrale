...
credential_path = os.path.join(os.getcwd(), "credentials/")
ISSUER_ID: str = json.load(
    open(os.path.join(credential_path, "wallet_credentials.json"))
)["issuer_id"]


class KragSentralePassIssuer:
    def __init__(self):
        self.key_file_path = os.environ.get(
            "GOOGLE_APPLICATION_CREDENTIALS",
            open(
                os.path.join(
                    credential_path, "automatic-entrance-pass-517489c6f6a2.json"
                )
            ),
        )
    ...
    # [START createClass]
    def create_class(self, class_suffix: str) -> str:
        """Create a class.
    ...
    # [START updateClass]
    def update_class(self, class_suffix: str) -> str:
        """Update a class.
        ...
        **Warning:** This replaces all existing class attributes!
        ...
        updated_class["reviewStatus"] = "DRAFT"

        response = (
            self.client.genericclass()
            .update(resourceId=f"{ISSUER_ID}.{class_suffix}", body=updated_class)
            .execute()
        )
    ...
    # [START patchClass]
    def patch_class(self, class_suffix: str) -> str:
        """Patch a class.
        ...
        response = (
            self.client.genericclass()
            .patch(resourceId=f"{ISSUER_ID}.{class_suffix}", body=patch_body)
            .execute()
        )
    ...
    # [START createObject]
    def create_object(self, class_suffix: str, object_suffix: str) -> str:
        """Create an object.
    ...
    # [START updateObject]
    def update_object(self, object_suffix: str) -> str:
        """Update an object.
        ...
        **Warning:** This replaces all existing object attributes!
        ...
        try:
            response = (
                self.client.genericobject()
                .get(resourceId=f"{ISSUER_ID}.{object_suffix}")
                .execute()
            )
        except HttpError as e:
            if e.status_code == 404:
        ...
        response = (
            self.client.genericobject()
            .update(resourceId=f"{ISSUER_ID}.{object_suffix}", body=updated_object)
            .execute()
        )
    ...
    # [START patchObject]
    def patch_object(self, object_suffix: str) -> str:
        """Patch an object.
        ...
        response = (
            self.client.genericobject()
            .patch(resourceId=f"{ISSUER_ID}.{object_suffix}", body=patch_body)
            .execute()
        )
    ...
    # [START jwtNew]
    def create_jwt_new_objects(self, class_suffix: str, object_suffix: str) -> str:
    ...
    # [START batch]
    def batch_create_objects(self, class_suffix: str):
